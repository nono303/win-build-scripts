<?php
	...
	if(is_dir($argv[1] = str_replace("\\","/",$argv[1]))){
		...
		foreach(["pdb" => ["/"],"dll" => ["/../bin/","/../bin/ossl-modules/","/../bin/engines-3/","/../_php-ts/php[0-9]"]] as $k => $v){
			...
			$pdbordlllist[$k] = [];
			foreach($v as $pv)
				$pdbordlllist[$k] = array_merge($pdbordlllist[$k],array_map('basename', glob($argv[1].$pv.'*.'.$k, GLOB_BRACE)));
