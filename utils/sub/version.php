<?php
	...
	$nogit = array(
		...
		"brotli"			=> ["/#define BROTLI_VERSION_MAJOR *([0-9]+).*#define BROTLI_VERSION_MINOR *([0-9]+).*#define BROTLI_VERSION_PATCH *([0-9]+)/s",
							PATH_SRC."/".$argv[1]."/c/common/version.h"],
		...
		"mod_fcgid"			=> ["/#define MODFCGID_VERSION_MAJOR *([0-9]+).*#define MODFCGID_VERSION_MINOR *([0-9]+).*#define MODFCGID_VERSION_SUBVER *([0-9]+).*#define MODFCGID_VERSION_DEV *([0-9]+)/s",
							PATH_SRC."/".$argv[1]."/modules/fcgid/fcgid_conf.h"],
		...
		"apr-util"			=> ["/#define APU_MAJOR_VERSION *([0-9]+).*#define APU_MINOR_VERSION *([0-9]+).*#define APU_PATCH_VERSION *([0-9]+)/s",
							PATH_SRC."/".$argv[1]."/include/apu_version.h"],
		"openssl-quic"		=> ["/VERSION_NUMBER=([0-9\.]+)/",
							PATH_SRC."/".$argv[1]."/makefile"],
		"verpatch"			=> ["/set _ver=\"([^ ]+)/",
							PATH_SRC."/".$argv[1]."/ver-self.cmd"],
		"php-geos"			=> ["/PHP_GEOS_VERSION \"([0-9\.]+)/",
							PATH_SRC."/".$argv[1]."/php_geos.h"],
		"pecl-text-xdiff"	=> ["/PHP_XDIFF_VERSION \"([0-9\.]+)/",
							PATH_SRC."/".$argv[1]."/php_xdiff.h"],
		"pecl-memcache"		=> ["/PHP_MEMCACHE_VERSION \"([0-9\.]+)/",
							PATH_SRC."/".$argv[1]."/src/php_memcache.h"],
		"icu"				=> ["/U_ICU_VERSION \"(.*)\"/",
							PATH_SRC."/".$argv[1]."/icu4c/source/common/unicode/uvernum.h"],
		"nssm"				=> ["/NSSM_VERSIONINFO (.*),(.*),(.*),(.*)/",
							PATH_SRC."/".$argv[1]."/version.h"],
		"libyuv"			=> ["/LIBYUV_VERSION (.*)/",
							PATH_SRC."/".$argv[1]."/include/libyuv/version.h"],
		"mobac"				=> ["/mobac.revision=(.*)/",
							PATH_SRC."/".$argv[1]."/mobac/build/resources/main/mobac/mobac-rev.properties"],
		...
		"serf"				=> ["/#define SERF_MAJOR_VERSION *([0-9]+).*#define SERF_MINOR_VERSION *([0-9]+).*#define SERF_PATCH_VERSION *([0-9]+)/s",
							PATH_SRC."/".$argv[1]."/serf.h"],
		"wineditline"		=> ["/RL_READLINE_VERSION *0x([0-9]{2})([0-9]{2})([0-9]{2})/s",
							PATH_SRC."/".$argv[1]."/src/editline/readline.h"],
	...
	if(sizeof($argv) == 1){
		...
		echo getVersion($argv[1])["product"];
