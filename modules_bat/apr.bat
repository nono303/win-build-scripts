...
cd /D C:\src\apr-util
"C:\msvc%MSVC_VER%\MSBuild\15.0\Bin\MSBuild.exe" aprutil.sln /nowarn:msb4011 /nowarn:C4244 /nowarn:C4098 /nowarn:C4267 /nowarn:C4311 /nowarn:C4018 /nowarn:C4133 /p:Turbo=true /m:8 /p:CL_MPCount=8 /t:apr;apr_crypto_openssl;apr_dbd_odbc;apr_ldap;aprapp;apriconv;aprutil;libapr;libaprapp;libapriconv;libapriconv_ccs_modules;libapriconv_ces_modules;libaprutil;preaprapp;preapriconv;preaprutil;prelibaprapp /p:Configuration=Release /p:DebugSymbols=true /p:DebugType=None /p:Platform="%ARCH%"
...
REM Copy /Y C:\httpd-sdk\install\bin\apr_crypto_openssl-1.pdb D:\github\NONO_subversion\vc%MSVC_VER%\%ARCH%%AVXB%\apr_crypto_openssl-1.pdb
REM Copy /Y C:\httpd-sdk\install\bin\apr_crypto_openssl-1.dll D:\github\NONO_subversion\vc%MSVC_VER%\%ARCH%%AVXB%\apr_crypto_openssl-1.dll