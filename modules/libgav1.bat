...
MSBuild.exe libgav1.sln %MSBUILD_OPTS% ^
/t:libgav1_shared ^
/p:Configuration=%CMAKE_BUILD_TYPE% ^
/p:Platform=%archmsbuild%

for %%Y in (dll pdb) do (xcopy /C /F /Y  %PATH_BUILD%\%1\%CMAKE_BUILD_TYPE%\libgav1.%%Y %PATH_INSTALL%\bin\*)
xcopy /C /F /Y  %PATH_BUILD%\%1\%CMAKE_BUILD_TYPE%\libgav1.lib %PATH_INSTALL%\lib\*
