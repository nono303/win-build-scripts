...
call do_php %PATH_UTILS%\sub\version.php libconfig %PATH_GITHUB_SSLH%\x%SSLH_TARGET_ARCH%\%AVXECHO%\cygconfig-11.dll libconfig
...
for %%A in (echosrv.exe sslh-ev.exe sslh-fork.exe sslh-select.exe) do (
	...
	call do_php %PATH_UTILS%\sub\version.php %1 %PATH_GITHUB_SSLH%\x%SSLH_TARGET_ARCH%\%AVXECHO%\%%A libconfig:%LIBCONFIG_VER%
)
