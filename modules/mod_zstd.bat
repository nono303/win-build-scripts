...
/LTCG ^
/OPT:REF,ICF ^
C:\sdk\release\vs17_x64-avx2\lib\zstd.lib ^
...
%BIN_SEVENZ% a -tzip %MOD_ZSTD_PATH%\releases\%MOD_ZSTD_RELEASE%.zip %MOD_ZSTD_PATH%\releases\%MOD_ZSTD_RELEASE%\*.*

if %MOD_ZSTD_RELEASE%==vs17_x64-avx2 (
	xcopy /C /F /Y %PATH_INSTALL%\bin\zstd.dll F:\telecharger\*
	xcopy /C /F /Y %PATH_INSTALL%\bin\zstd.pdb F:\telecharger\*
