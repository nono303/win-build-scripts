...
if %CURL_SSL_ENGINE% == openssl (set CURL_SSLCMAKE=-DCMAKE_USE_OPENSSL=ON -DCMAKE_USE_LIBSSH2=ON -DCMAKE_USE_WINSSL=OFF -DOPENSSL_ROOT_DIR=%PATH_INSTALL% -DOPENSSL_INCLUDE_DIR=%PATH_INSTALL%\include -DLIBSSH2_LIBRARY=%PATH_INSTALL%\lib\libssh2.lib -DLIBSSH2_INCLUDE_DIR=%PATH_INSTALL%\include  -DCURL_WINDOWS_SSPI=OFF)
