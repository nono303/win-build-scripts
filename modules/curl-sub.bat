...
if %CURL_SSL_ENGINE% == winssl (set CURL_SSLCMAKE=-DCURL_USE_OPENSSL=OFF -DCURL_USE_LIBSSH2=OFF -DCMAKE_USE_SCHANNEL=ON -DCURL_WINDOWS_SSPI=ON)
if %CURL_SSL_ENGINE% == openssl (set CURL_SSLCMAKE=-DCURL_USE_OPENSSL=ON -DCURL_USE_LIBSSH2=ON -DCMAKE_USE_WINSSL=OFF -DOPENSSL_ROOT_DIR=%PATH_INSTALL% -DOPENSSL_INCLUDE_DIR=%PATH_INSTALL%\include -DLIBSSH2_LIBRARY=%PATH_INSTALL%\lib\libssh2.lib -DLIBSSH2_INCLUDE_DIR=%PATH_INSTALL%\include  -DCURL_WINDOWS_SSPI=OFF)
